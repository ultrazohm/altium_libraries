...
CREATE OR REPLACE TEMPORARY TABLE altium_library.sample_table (
	...
	`Footprint Path 2` VARCHAR(100) NOT NULL,
	`Footprint Path 3` VARCHAR(100) NULL,
	`Footprint Ref` VARCHAR(100) NULL,
...
DELIMITER //
CREATE OR REPLACE PROCEDURE create_table (table_name VARCHAR(64))
BEGIN
    SET @SQL := CONCAT('CREATE OR REPLACE TABLE ', table_name, 
   					   '(ID INT NOT NULL AUTO_INCREMENT, CONSTRAINT table_names_pk PRIMARY KEY (ID))');
...
BEGIN
	...
	FOR record IN (SELECT table_name AS name FROM table_names) DO
		...
		SET @SQL := CONCAT('CREATE OR REPLACE TABLE ', name, ' LIKE sample_table');
  		PREPARE stmt FROM @SQL;
    	EXECUTE stmt;
   		DEALLOCATE PREPARE stmt;
	END FOR;
...
DELIMITER //
CREATE OR REPLACE PROCEDURE drop_tables_from_def ()
BEGIN
	DECLARE name VARCHAR(64);
	FOR record IN (SELECT table_name AS name FROM table_names) DO
		# Entries in record.name contain white spaces which must be escaped using `...`
...
#CALL drop_tables_from_def();


DROP TABLE IF EXISTS table_names;
DROP TABLE IF EXISTS default_columns;
